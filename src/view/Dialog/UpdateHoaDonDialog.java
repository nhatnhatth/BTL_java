...
public class UpdateHoaDonDialog extends JDialog {
    ...
    public UpdateHoaDonDialog(JFrame parent, HoaDon hd, Callback callback) {
        super(parent, "Update", false);
        ...
        for (SanPham item : spList) {
            ...
            if(hd.getMasp() == item.getMasp()){
                model.setSelectedItem(item.getTensp());
            }
        }
        ...
        for (NhanVien item : nvList) {
            ...
            if(hd.getManv() == item.getMaNV()){
                model2.setSelectedItem(item.getTenNV());
            }
        }
        ...
        for (KhachHang item : khList) {
            ...
            if(hd.getMakh() == item.getMaKH()){
                model3.setSelectedItem(item.getTenKH());
            }
        }
        ...
        jDateChooser.setDate(hd.getDate());
        ...
        slField.setText(String.valueOf(hd.getSoluong()));
        ...
        themButton.addActionListener(e -> {
            ...
            callback.update(new HoaDon(hd.getMahd(), maNV, maKH, maSP, ngaynhap, thanhTien, soluong));
            dispose();
        });
